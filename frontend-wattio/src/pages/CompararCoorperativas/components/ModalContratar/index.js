...
export default function ModalContratar({mostrar,setMostrar,nome}){
  return(
    <Container className='mt-4'>
      <Modal
        ...
        backdrop="static"
        keyboard={false}
        onHide={() => setMostrar({mostrar: false})}
      ...
      >
        ...
        <Modal.Body style={{backgroundColor: 'rgba(5, 5, 130, .8)'}}>
          Enviaremos o pedido para a coorperativa <span className='text-warning'>"{nome}"</span> e em seguida entraremos em contato com você.
        </Modal.Body>
        <Modal.Footer style={{backgroundColor: 'rgba(5, 5, 130, .8)'}}>
          <Button variant="success" onClick={() => setMostrar({mostrar:false})}>
            Entendi
          </Button>
        </Modal.Footer>
      </Modal>
